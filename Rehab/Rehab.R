...
#File Read 
result = fromJSON(file =  "/Users/omachowda/Desktop/Messenger Analysis/messages/inbox/RehabKhan_UN6XWmBv4w/message.json")
#Data Head
...
df <- data.frame(
  sender = factor(c('Me', 'You'),
                  levels = c('Me', 'You')),
  value = c(as.numeric(as.character(tbl['Omar Choudhry'])),
            as.numeric(as.character(tbl['Rehab Khan']))))
...
bp + 
  ...
  minimalplottheme +
  geom_text(aes(y = c(75000,25000),
                label = value),
            size=6,
            colour='black') +
  scale_fill_manual("legend", values = c("Me" = "#7ed6ce", "You" = "#ff9ea4"))+
  labs(fill = "Sender")+ 
...
ggplot(mydf,
  ...
  scale_fill_manual("legend", values = c("Omar Choudhry" = "#7ed6ce", "Rehab Khan" = "#ff9ea4"), labels=c("Me", "You"))+
...
wordfreq = as.data.frame(sort(table(y), decreasing=T)[2:501])
wordfreq = as.data.frame(sort(table(y), decreasing=T)[c("babe","sorry","love","awwh","food","morning","happy","cute","miss","hi")])

ggplot(wordfreq, aes(y, Freq)) +
  ggtitle('Most Frequent Words') +
  geom_col(fill=scale_fill_brewer(palette='RdPu')$palette(6)[2])+
