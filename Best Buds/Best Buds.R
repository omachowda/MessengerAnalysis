...
#File Read 
result = fromJSON(file =  "/Users/omachowda/Desktop/Messenger Analysis/messages/inbox/Highforce_tbMFZx2JWg/message.json")
#Data Head
...
bp + 
  ...
  minimalplottheme +
  geom_text(aes(y = rev(c(1700,6000,11000,15000,18000)),
                label = value),
            size=6,
            colour='black') +
...
mytext = mydf$content
y = na.omit(unlist(strsplit(as.character(mytext), " ")))
wordfreq = as.data.frame(sort(table(y), decreasing=T)[1:10])
